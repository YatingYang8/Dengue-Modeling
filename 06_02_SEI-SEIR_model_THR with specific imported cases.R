...
seiseir_model_thr <- function(t, state, parameters) {
  with(as.list(c(state,parameters)), {
    ...
    dS <- -a(temp[t])*b(temp[t])*(M3/(S+E+I+R))*S + BR*(S/1000)/365 - DR*(S/1000)/365 + ie*(S+E+I+R) - ie*S
    dE <- a(temp[t])*b(temp[t])*(M3/(S+E+I+R))*S-(1.0/5.9)*E - DR*(E/1000)/365 - ie*E
    dI <- (1.0/5.9)*E-(1.0/5.0)*I - DR*(I/1000)/365 - ie*I
    
    # 添加输入病例
    dI <- dI + imported_cases[t]
